...
<script>
	function evaluar(idClase, status) {
		if (status == '0') {
			${remoteFunction(controller: 'califica', action: 'calificar', id: 1)}
		} else if(status=='2'){
	...
	<g:if test="${session.alumno}">
		...
		<g:form id="formulario" name="formulario" action="save">
			<fieldset class="form">
				...
				<br>
				<strong>Matrícula:</strong> ${session.alumno.matricula}
				<br>
				<strong>Nombre:</strong>	${session.alumno.nombre} ${session.alumno.paterno} ${session.alumno.materno}
				<br>
				<strong>Carrera: </strong>
				
				<g:if test="${clases.get(0)}">
				...
				<table>
					...
					<g:each var="clase" in="${clases}" status="i">
						<tr onClick="evaluar(${clase.id}, ${evaluaciones.get(i)})">
							...
							<td>
								<g:if test="${evaluaciones.get(i)=='0'}">
									<img src="${resource(dir: 'Imagenes', file: 'tache.png')}" width="25" height="25">
								</g:if>
								<g:elseif test="${evaluaciones.get(i)=='1'}">
									<img src="${resource(dir: 'Imagenes', file: 'palomita.png')}" width="28" height="28">
								</g:elseif>
								<g:else>
									<img src="${resource(dir: 'Imagenes', file: 'noDisponible.png')}" width="25" height="25">
								</g:else>
