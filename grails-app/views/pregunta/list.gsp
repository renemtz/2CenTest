			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="descripcion" title="${message(code: 'pregunta.descripcion.label', default: 'Descripcion')}" />
					
						<th><g:message code="pregunta.evaluacion.label" default="Evaluacion" /></th>
				...
				<g:each in="${preguntaInstanceList}" status="i" var="preguntaInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${preguntaInstance.id}">${fieldValue(bean: preguntaInstance, field: "descripcion")}</g:link></td>
					
						<td>${fieldValue(bean: preguntaInstance, field: "evaluacion")}</td>
